...
function Yemot_router(opt = { timeout: 0 }) {
	...
	router.add_fn = function (path, fn) {
		...
		this.all(path, (req, res, next) => {
			...
			if (!check_request(req.query)) {
				return res.json({ message: "request not valid" });
			}
	...
	async function new_call(fn, call_id, call) {
		...
		try {
			...
			} else if (error instanceof Exit_error) {
				on_exit(call_id);
	...
	function on_exit(call_id) {
		delete active_calls[call_id];
		console.log(call_id, "exit");
	}
...
Yemot_router.errors = { Hangup_error, Timeout_error, Exit_error };

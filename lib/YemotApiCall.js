 ...
 * @param {*} events
 */
const YemotApiCall = function (call_id, events) {
	...
	this.read = async function (massage, mode = "tap", options = {}) {
		...
		let [response_txt, val_return] = functions.make_read_response(massage, mode, options);
	...
	this.id_list_message = function (data) {
		...
		let response_txt = functions.make_id_list_message_response(data);
